...
EXPECTED_SUCCESS_OUTPUT = '<span title="Exports" alt="Exports" id="OthDimItem2">Exports</span>'
...
def test_read_updated_webpage_content_missing_params():
    response = read_updated_webpage_content(url=URL, select_menu_id="",
...
def test_read_updated_webpage_content_success():
    reponse = read_updated_webpage_content(url=URL, select_menu_id=SELECT_MENU_ID,
                                           select_menu_value_id=SELECT_MENU_VALUE_ID)
    assert EXPECTED_SUCCESS_OUTPUT in reponse


def test_read_updated_webpage_content_wrong_select_menu_id():
    response = read_updated_webpage_content(url=URL, select_menu_id=WRONG_SELECT_MENU_ID,
                                            select_menu_value_id=SELECT_MENU_VALUE_ID)
    assert EXPECTED_WRONG_SELECT_MENU_ID == response


def test_read_updated_webpage_content_wrong_select_menu_value_id():
    response = read_updated_webpage_content(url=URL, select_menu_id=SELECT_MENU_ID,
